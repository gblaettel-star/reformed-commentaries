/* esv-links.js — shared scripture linkifier for the study section.
   Turns "Book chapter:verse" references in page text into ESV.org links,
   matching the commentary. Self-contained: injects its own CSS and auto-runs
   (including on JS-rendered / re-rendered content). Extracted verbatim from
   the commentary's linkifyScripture + ESV_BOOKS. */
(function () {
  var _st = document.createElement('style');
  _st.textContent = '.scripture-link{color:inherit;text-decoration:underline;text-decoration-color:rgba(201,168,76,.45);text-decoration-style:dotted;text-decoration-thickness:1px;text-underline-offset:3px}.scripture-link:hover{text-decoration-color:#c9a84c}';
  ...
  function _run() {
    ...
    try { linkifyScripture(document.body); } catch (e) {}
    if (_obs) _obs.observe(document.body, { childList: true, subtree: true });
  }
  var _t = null;
  function _schedule() { clearTimeout(_t); _t = setTimeout(_run, 150); }
  function _init() {
    if (window.MutationObserver) { _obs = new MutationObserver(_schedule); }
    _run();
  }
